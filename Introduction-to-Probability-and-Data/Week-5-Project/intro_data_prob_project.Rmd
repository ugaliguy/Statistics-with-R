...
For both males and females there appears to be a slight negative correlation.

**Research quesion 3:**

```{r}
set2 <- select(brfss2013, misphlpf, mishopls, miswtles, sex, genhlth, sleptim1) %>%
   filter(misphlpf <= 9) %>% filter(mishopls <=5) %>% filter(miswtles != "NA") %>% filter(sex != "NA") %>% filter(genhlth != "NA") %>% filter(sleptim1 <= 12)
summary(set2)
q <- ggplot(set2, aes(genhlth, mishopls))
q + geom_jitter(alpha= 1/2,aes(fill = mishopls)) + scale_y_continuous(limits = c(4,10), breaks = 5:9) +  facet_grid(.~misphlpf) + theme(text = element_text(size=20), axis.text.x = element_text(angle=90, vjust=1)) + xlab("genhlth = Self-Reported General Health") + ylab ("sleptim1 = Number of Hours of Sleep?") + theme_bw()
```
